...
function create(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        bb.findOne({ email: req.body.email }, (err, docs) => {
            if (err) {
...
function deletaimages(id__) {
    console.log(id__);
    _file.findOneAndDelete({ _id: id__ }, function (err, docs) {
        if (err) {
            console.log(err);
        }
        else {
        ...
        if (docs) {
            ...
            if (docs != null) {
                docs.forEach((e) => {
                    ...
                    if (e.files_id == id__) {
                        _chunk.findOneAndDelete({ _id: e._id }, function (err, docs) {
                            if (err) {
                                console.log(err);
                            }
                            else {
...
function authuser(req, res, next) {
    ...
    bb.findOne({ email: req.body.email }, (err, docs) => {
        ...
        if (docs != null) {
            console.log(docs, "docs"), bod1 = docs;
            if (req.body.password == bod1.password) {
                req.body.password = "";
                let token = jwt.sign(req.body, key);
                res.cookie("oblivion", token, {
                    maxAge: 900000,
...
function withdrawandfund(req, res, next) {
    ...
    jwt.verify(_cookie, key, (err, data) => {
        ...
        let cookiedata = data;
        let user = data.email;
        transb.findOne({ email: user }, (err, data) => {
            let transbdata = data;
            if (err) {
                res.status(404).send(err);
                return;
